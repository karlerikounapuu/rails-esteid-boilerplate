module MobileIdAuthenticatable
  ...
  def broadcast_success_to_channel!
    ...
    redirect_uri = Rails.application.routes.url_helpers.authorize_eid_session_path(Authentication.last.id)
    redirect_uri = "http://localhost:3000#{redirect_uri}"
    ActionCable.server.broadcast(
      "authentication_session_#{id}",
  ...
  def wait_mid_result!
    ...
    self.state = 'AUTH_SUCCEEDED' if body['state'] == 'COMPLETE' && body['result'] == 'OK'

    save!

    state == 'AUTH_SUCCEEDED' ? broadcast_success_to_channel! : broadcast_user_error_to_channel!
